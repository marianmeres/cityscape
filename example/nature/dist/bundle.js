...
class FlyerDirector {
	...
	#spawn() {
		...
		const speed = type === "balloon"
			? this.#rng.float(0.00004, 0.00008)
...
function drawBroadleaf(out, x, groundY, w, h, spec, c, sway) {
	...
	if (c.snow > 0.02) {
		out.circle(
			...
			cyTop - r1 * 0.36,
			r1 * 0.7,
			withAlpha(c.snowColor, c.snow * 0.55),
		);
	}
}
...
function drawShrub(out, x, groundY, w, h, spec, c, sway) {
	...
	if (c.snow > 0.02) {
		out.circle(
			...
			groundY - r1 * 1.3,
			r1 * 0.7,
			withAlpha(c.snowColor, c.snow * 0.5),
		);
	}
}
		...
		if (arming) {
			if (flushing) {
				if (++chainDepth > 1000) {
					chainDepth = 0;
					throw new Error(
						`vanilla: maximum update depth exceeded (${1000}) — ` +
