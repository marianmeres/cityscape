...
| 8    | Coast biome (sparse waterfront)                           | [02](./02-biome-journey.md) #3     | ✅ `88cf381`                      |
| 9    | Transition blending + far-layer hills/treeline            | [02](./02-biome-journey.md) #4     | ✅ `88cf381`                      |
...
| 12   | Neon rooftop signs                                        | [04](./04-atmosphere-polish.md) #2 | ✅ `d0695fe`                      |
| 13   | Water reflections of near-window lights                   | [04](./04-atmosphere-polish.md) #3 | ✅ `59dde70`                      |
| 14   | Drifting airship                                          | [04](./04-atmosphere-polish.md) #4 | ✅ `5770da5`                      |
| 15   | Aurora ribbon                                             | [04](./04-atmosphere-polish.md) #5 | ✅ `5f8e038`                      |
| 16   | Re-tune the curated default snapshot                      | [00](./00-overview-and-roadmap.md) | ✅ owner-supplied config          |
...
- **2026-06-20** — Tasks 8/9 (coast + far-layer hills) complete the journey arc (city → … → countryside → coast → back). New biome-only `coast` district + a `hill`/`mound` shape routed only to the far bands (near bands downgrade it to a tree). Verified: `variety=0` **byte-identical** to the pre-task commit (worktree comparison, 240 frames); far-band routing, coast gating, zero rural leak, no NaN. Both off by default (need `biomeVariety > 0`).
- **2026-06-20** — Tasks 12–15 (neon signs, water window-reflections, airship, aurora) implemented + probed (gating, NaN, determinism + DOM-purity via the suite). All **ship on at moderate defaults** (neon 0.4, aurora 0.3 navy/vaporwave-only, fog 0.3, traffic 0.6) per the established "subtly on, re-tune at 16" pattern. ⚠️ **neon, aurora, and the extra water reflections were added _after_ the owner's last visual review** — flag for the task-16 re-tune; dial any down via their knobs.
- **2026-06-20** — Task 16 **done** — the owner supplied a curated config (via a permalink); baked it into `CONFIG_SCHEMA` defaults verbatim (verified `DEFAULT_CONFIG` matches exactly). Notably: palette **dawn**, `biomeVariety 0.3` (journey **on** by default), `vignette 1`, `darkness 0.76`, `windowToggleRate 0.48`, `cameraSpeed 120`, `waterLevel 0.1`, fog/aurora **0**, traffic 0.1. The example now mounts with `randomizeSeed: false` so a bare load shows this exact curated scene (seed "hey ho lets go").
- **2026-06-20** — Added an **immersive fullscreen** view (button + `f` key): hides all controls and goes browser-fullscreen; Escape (or `f`) restores. In `example/main.ts`.
- **2026-06-20** — Fixed an **animation-loop resume glitch**: when a window is occluded without firing `visibilitychange` (alt-tabbed / covered), the loop kept running throttled and jumped on return. The engine now treats any frame gap > `maxFrameDeltaMs` as a wake-up and resumes with **zero** catch-up (repaint, advance nothing). Regression tests in `tests/engine.test.ts`.
