/**
 * The headless naturescape scene — the top-level, renderer-agnostic, DOM-free API.
 *
 * `createNaturescape(config)` builds the whole simulation (camera, mood engine, parallax landscape,
 * ambient director) and returns a {@link NatureScene} you drive with `update(dt)` and read with
 * `collect(w, h)` → a {@link DisplayList}. Hand that list to *any* {@link Renderer} (Canvas, ASCII,
 * PixelArt). The orchestration order matters and lives here: step the camera, resolve the mood, run
 * the spawners against the new camera position, then update entities. This mirrors the cityscape's
 * scene exactly — same engine, same seam, different domain.
 *
 * Determinism: everything derives from `config.seed`. The same seed + config reproduces the exact
 * landscape — which is what the tests rely on and what the URL-hash permalink restores.
 *
 * @module
 */
...
import { DEFAULT_CONFIG, type NatureConfig, normalizeConfig } from "./config.ts";
import { MoodEngine } from "./mood.ts";
import { AmbientDirector, AmbientEventBus } from "./events.ts";
import type { NatureEnv } from "./env.ts";
import { buildLandscape } from "./generation/landscape.ts";
...
export interface NatureScene {
	...
	readonly world: World<NatureEnv>;
	/** The live, normalised configuration (mutated by {@link NatureScene.setConfig}). */
	readonly config: NatureConfig;
	...
	setConfig(patch: Partial<NatureConfig>): void;
	/** A pointer-parallax sway in screen px — horizontal `px` and (optional) vertical `py`. */
	setSway(px: number, py?: number): void;
	/** Light interaction: drop a ripple on the water under a screen point. */
	poke(screenX: number, screenY: number): void;
}

class Naturescape implements NatureScene {
	config: NatureConfig;
	...
	constructor(config: NatureConfig) {
		this.config = config;
		this.#build();
	}

	get world(): World<NatureEnv> {
		return this.#world;
	}
	...
	#build(): void {
		...
		this.#world = new World<NatureEnv>(
	...
	resize(width: number, height: number): void {
		...
		// frame before its first fixed `update()` step (a 0ms first delta dispatches no steps).
...
function signedSpeed(config: NatureConfig): number {
	return config.cameraSpeed * (config.cameraDirection === "left" ? -1 : 1);
}

/**
 * Create a headless naturescape simulation. Pass any subset of {@link NatureConfig}; the rest fills
 * from {@link DEFAULT_CONFIG} and everything is clamped to valid ranges.
 */
export function createNaturescape(
	config: Partial<NatureConfig> = {},
): NatureScene {
	return new Naturescape(normalizeConfig({ ...DEFAULT_CONFIG, ...config }));
}
