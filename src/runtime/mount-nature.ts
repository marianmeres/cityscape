/**
 * The browser runtime for the naturescape — the one place that assembles the whole nature scene
 * for a page.
 *
 * `mountNaturescape()` creates a full-page canvas, wires a {@link CanvasRenderer}, builds the
 * headless scene, drives it with an rAF-backed {@link Engine}, and adds the optional light
 * interaction (pointer-parallax sway, wheel-scrub speed, click-to-ripple) and ambient audio. It
 * returns a {@link NatureHandle} the example (or any host) uses to swap renderers, push config
 * changes, and read a permalink. All DOM lives here or in the renderer/ui/audio — the engine and
 * naturescape stay pure. A faithful sibling of `mountCityscape`.
 *
 * @module
 */
...
import { NatureAudio } from "../audio/nature-audio.ts";

/** Options for {@link mountNaturescape}. */
export interface NatureMountOptions {
...
/** The live handle returned by {@link mountNaturescape}. */
export interface NatureHandle {
	readonly scene: NatureScene;
	readonly engine: Engine;
	readonly canvas: HTMLCanvasElement;
	readonly audio: NatureAudio;
	...
	update(patch: Partial<NatureConfig>): void;
	/** Subscribe to runtime-initiated config changes (wheel scrub) to refresh a panel. */
	onConfigChange(fn: (config: NatureConfig) => void): () => void;
...
/** Mount a running naturescape onto the page. */
export function mountNaturescape(opts: NatureMountOptions = {}): NatureHandle {
	...
	let initial: Partial<NatureConfig> = { ...opts.config };
	...
	const configListeners = new Set<(c: NatureConfig) => void>();
	...
	const applyRuntime = (patch: Partial<NatureConfig>): void => {
	...
	const update = (patch: Partial<NatureConfig>): void => {
